...
export function ChapterGoaTractors() {
  ...
  useGSAP(() => {
    ...
    if (!section) return;
    gsap.from(section.querySelectorAll(".goa-label, .goa-title, .goa-copy, .campaign-card"), {
      y: 50,
      opacity: 0,
      stagger: .1,
      scrollTrigger: { trigger: section, start: "top 72%", end: "top 28%", scrub: true }
    });
    gsap.to(section.querySelector(".tractor-wheel"), {
      rotate: 360,
